...
function useGetLpList({cursor, search, order, limit}:PaginationDto) {
  return useQuery({
    ...
    // 데이터가 신선하다고 간주하는 시간
    // 이 시간 동안은 캐시된 데이터를 그대로 사용함. 컴포넌트가 마운트 되거나 창에 포커스 들어오는 경우에도 재요청 안함.
    ...
    // keepPreviousData: 파라미터가 변경될 때 이전 데이터를 유지하여 UI 캄빡임(FLicking)을 줄여줌.
    // 페이지네이션 시 페이지 전환 사이에 이전 데이터를 보여주어 사용자 경험을 향상시킴.
