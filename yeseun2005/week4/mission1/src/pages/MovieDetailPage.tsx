...
export default function MovieDetailPage() {
  ...
  if (error) return <p className="p-8 text-red-400">❌ 데이터를 불러오는 중 오류가 발생했습니다.</p>;
  if (!movie) return <p className="p-8 text-white">❌ 영화 정보를 불러올 수 없습니다.</p>;

  const cast = credits?.cast ?? [];
  const crew = credits?.crew ?? [];

  return (
    <div className="min-h-screen bg-black flex flex-col items-center text-white p-8">
